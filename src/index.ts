...
import { copyTemplate, setupDatabase, installDependencies, initGit } from "./utils/template";
import { type ProjectConfig, type TemplateType } from "./types";
...
const getAvailableTemplates = async (templatePath: string): Promise<string[]> => {
    try {
...
const runCli = async () => {
    ...
    .argument("[dir]", "The directory for the project (default: current directory)", ".")
    .option("-d, --database <type>", "Database type (postgres/sqlite/mysql)", "postgres")
    ...
    try {
        ...
        const projectName = directory === "." ? path.basename(process.cwd()) : path.basename(projectPath);
